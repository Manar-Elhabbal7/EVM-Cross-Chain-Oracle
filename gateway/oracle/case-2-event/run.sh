...
cd "$SCRIPT_DIR"
RPC_A=$RPC_A RPC_B=$RPC_B PRIVATE_KEY=$PRIVATE_KEY ADDRESS_A=$ADDRESS_A ADDRESS_B=$ADDRESS_B node event-sync.js > oracle.log 2>&1 &
ORACLE_PID=$!
...
echo "Step 2: Waiting for event propagation (5s)..."
sleep 5
